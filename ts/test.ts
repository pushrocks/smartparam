...
var existsTest1 = function() {
    if(!smartparam.exists(parentObject,"childObject")) {
        bl.success(
            "childObject is not a child of parentObject at this time. " +
            "Perfect!"
        );
    } else {
        bl.error(
            "childObject should not be a child of parentObject at this time." +
...
var existsTest2 = function() {
    if(smartparam.exists(parentObject,"childObject")) {
        bl.success(
            "childObject is a child of parentObject at this time. " +
            "Perfect!"
        );
    } else {
        bl.error(
            "childObject should be a child of parentObject at this time. " +
            "It seems something is wrong");
        process.exit(1);
    };
};
